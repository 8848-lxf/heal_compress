...
class StrictGAConfig:
    ...
    def __post_init__(self) -> None:
        if not 0.0 < float(self.target_bops_retention) <= 1.0:
            raise ValueError("formal_ga_target_bops_retention_invalid")
        if not 0.0 <= float(self.tolerance_abs) <= 0.005:
    ...
    def __init__(
        ...
        *,
        ...
        if self.include_activation_taylor and activation_cache is None:
            raise ValueError("activation_taylor_enabled_but_cache_missing")
        ...
        if not feasible and self.enforce_bops_hard_gate:
            result = {
                **identity,
                ...
                "activation_taylor_included": self.include_activation_taylor,
                "genotype": genotype,
...
def score_stage2(
    ...
    *,
    ...
    if not 0.0 <= tolerance <= 0.005:
        raise ValueError("stage2_accuracy_tolerance_must_not_exceed_0_005")
    if float(greedy_p50_ms) <= 0.0:
        raise ValueError("stage2_greedy_latency_must_be_positive")
    eligible = bool(
        result.deployable
        and float(result.map) >= float(greedy_map) - tolerance
    )
    if not eligible:
    ...
    accuracy = (
        ...
        if tolerance == 0.0
            ...
            -1.0,
            min(1.0, (float(greedy_map) - float(result.map)) / tolerance),
        )
    )
        ...
        *,
        ...
        for generation in range(1, self.config.generations + 1):
            ...
            generation_winner, stage2_rows = select_generation_winner(
                ...
                accuracy_tolerance=self.config.stage2_accuracy_tolerance,
            )
            anchors.update(
                new_results,
                greedy_map=float(greedy_anchor.map),
