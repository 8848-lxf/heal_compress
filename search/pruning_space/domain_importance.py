    ...
    *,
    ...
    for unit_id in sorted(unit_to_parameter_slices):
        ...
        for parameter_name, rows in sorted(by_parameter.items()):
            ...
            removed = _removed_mask(weight, rows)
            count = int(removed.sum().detach().cpu())
            if count <= 0:
                continue
            gradient = gradient.detach().to(device=weight.device, dtype=weight.dtype)
            fisher = fisher.detach().to(device=weight.device, dtype=weight.dtype)
