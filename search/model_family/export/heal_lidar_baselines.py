...
Formal deployment starts at dense post-scatter BEV features. The historical
fixed-K wrapper remains available only for explicit legacy artifact inspection.
Invalid padded agents are masked so a one-agent sample remains numerically
equivalent to the original variable-agent forward.
"""
...
class HealLidarBaselineExportPolicy:
    ...
    def __post_init__(self) -> None:
        if self.fixed_k is not None and int(self.fixed_k) <= 0:
...
def prepare_heal_lidar_baseline_inputs(
    ...
    *,
    ...
    if policy.fixed_k is None:
        raise RuntimeError("legacy_fixed_k_export_requires_fixed_k")
    ...
    if voxel_count > int(policy.fixed_k):
        raise RuntimeError(
