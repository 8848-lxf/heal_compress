...
def _floating_element_type(precision: str) -> int:
    from onnx import TensorProto
    ...
    raise ValueError(f"unsupported_strongly_typed_floating_precision:{precision}")
...
def _validate_same_type_invariants(model: Any) -> list[dict[str, Any]]:
    """Return dtype conflicts that TensorRT strongly typed parsing would reject."""

    types = _propagate_known_types(model)
    conflicts: list[dict[str, Any]] = []
    for node in model.graph.node:
        op_type = str(node.op_type)
        if op_type in (_SAME_TYPE_ELEMENTWISE_OPS | _SAME_TYPE_COMPARISON_OPS):
            indices = range(len(node.input))
        elif op_type in {"Concat", "GridSample"}:
            indices = range(len(node.input))
        else:
            continue
        typed_inputs = [
            (int(index), str(node.input[index]), types.get(str(node.input[index])))
            for index in indices
            if str(node.input[index]) and types.get(str(node.input[index])) is not None
        ]
        distinct = sorted({int(dtype) for _, _, dtype in typed_inputs})
        if len(distinct) > 1:
            conflicts.append(
                {
                    "node": str(node.name),
                    "op_type": op_type,
                    "inputs": [
                        {"index": index, "tensor": tensor, "onnx_element_type": dtype}
                        for index, tensor, dtype in typed_inputs
                    ],
                    "distinct_onnx_element_types": distinct,
                }
            )
    return conflicts
...
def apply_strongly_typed_precision_contract(
    ...
    *,
    ...
    # ONNX shape inference does not always materialize dtypes for type-preserving
    # operators (notably Gather).  Populate those dtypes before the single-pass
    # rewrite so downstream elementwise closure cannot silently miss a conflict.
    _propagate_known_types(model)
    ...
    for node in model.graph.node:
        ...
        if entry is not None and str(entry.realized_request_precision) in {"fp16", "fp32", "bf16"}:
            target = _floating_element_type(str(entry.realized_request_precision))
        ...
        if str(auxiliary_precision) in {"fp16", "fp32", "bf16"}:
            target = _floating_element_type(str(auxiliary_precision))
            # Where input 0 is a boolean condition.  Only its two data branches
            # participate in the floating precision contract.
            auxiliary_inputs = (
                range(1, min(3, len(node.input)))
                if op_type == "Where"
                else range(len(node.input))
            )
            for input_index in auxiliary_inputs:
                _insert_input_cast(
                    model=model,
                    node=node,
                    input_index=input_index,
        ...
        if output_precision in {"fp16", "fp32", "bf16"}:
            target = _floating_element_type(output_precision)
    ...
    dtype_conflicts = _validate_same_type_invariants(model)
    if dtype_conflicts:
        first = dtype_conflicts[0]
        raise ValueError(
            "strongly_typed_dtype_invariant_failed:"
            f"{first['node']}:{first['op_type']}:{first['distinct_onnx_element_types']}"
        )
    destination = Path(output_onnx)
    destination.parent.mkdir(parents=True, exist_ok=True)
    onnx.save(model, str(destination))
    counts = {"INT8": 0, "FP16": 0, "FP32": 0, "BF16": 0}
    for entry in mapping.entries:
    ...
    return {
        ...
        "dtype_invariant_conflicts": dtype_conflicts,
        "dtype_invariant_conflict_count": len(dtype_conflicts),
