...
class HEALLiDARCoBEVTSignalMaxK(nn.Module):
    ...
    def forward(
        ...
        # The plugin emits the fixed ``max_cav`` batch.  In native HEAL the
        # PointPillar/backbone path only runs over real CAVs and ``Regroup``
        # pads missing agents *after* the shrinker.  Letting a padded all-zero
        # scatter plane traverse biased Conv/BN layers creates a non-zero fake
        # agent and changes CoBEVT even though its attention mask is false.
        # Restore the native semantic boundary before the geometric warp.
        valid_agents = (
            torch.arange(self.max_cav, device=record_len.device)
            < record_len.reshape(-1)[0]
        )
        feature = feature * valid_agents.reshape(self.max_cav, 1, 1, 1).to(feature)
