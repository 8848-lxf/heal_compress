...
def _toy_model(path: Path, *, with_agent_mask: bool = False) -> None:
    inputs = [
    ...
    if with_agent_mask:
        inputs.append(
            helper.make_tensor_value_info("agent_mask", TensorProto.FLOAT, [1, 2])
        )
    ...
    nodes = [
        ...
        helper.make_node(
            "Add",
            ["bev_mean", "pairwise_mean"],
            ["before_agent_mask" if with_agent_mask else "prediction"],
        ),
    ]
    if with_agent_mask:
        nodes.extend(
            [
                helper.make_node("ReduceMean", ["agent_mask"], ["agent_mean"]),
                helper.make_node(
                    "Add", ["before_agent_mask", "agent_mean"], ["prediction"]
                ),
            ]
        )
...
def test_externalize_scatter_retains_non_frontend_agent_mask(tmp_path: Path):
    source = tmp_path / "source.onnx"
    destination = tmp_path / "external.onnx"
    _toy_model(source, with_agent_mask=True)
    ...
    assert "agent_mask" not in report["removed_fixed_k_inputs"]
