...
from search.proxy.conservative_gate_activation_taylor import GateDomainScores, rerank_domains_by_gate_scores
...
def run(args: argparse.Namespace) -> int:
    ...
    gate_path = args.output_root / "proxy/structural_gate_mapping.json"
    if gate_path.is_file():
        gate_payload = json.loads(gate_path.read_text(encoding="utf-8"))
        gate_scores = {str(row["domain_id"]): GateDomainScores(str(row["domain_id"]), dict(row["unit_scores"]), str(row["semantic_root_tensor"]), str(row["gate_tensor"]), tuple(row["physical_dependencies"]), str(row["family"])) for row in gate_payload["domains"]}
        space = replace(space, pruning_domains=rerank_domains_by_gate_scores(space.pruning_domains, gate_scores))
    winner_path = args.winner_file or (args.output_root / "winner/v2xvit_greedy005_winner.json")
    ...
    for name, raw_candidate in controls.items():
        ...
        export = _export_candidate(destination, physical.model, adapter, batch, hypes, phenotype, candidate_id, physical.report.structure_hash, build_engine=True, tensorrt_root=args.tensorrt_root, plugin=args.plugin, calibration_frames=200, qkv_paths=qkv_paths, fixed_k_override=args.fixed_k, physical_gpu_id=args.physical_gpu)
...
def main() -> int:
    ...
    parser.add_argument("--winner-file", type=Path)
